...
def test_connection():
    ...
    remove_flag('fw-proxy.ready')
    try:
        who, _ = run('whoami')
    except MgmtNotConfigured as e:
        remove_flag('fw-proxy.configured')
        status_set('blocked', str(e))
    except subprocess.CalledProcessError as e:
        remove_flag('fw-proxy.configured')
        status_set('blocked', e.output)
    else:
        set_flag('fw-proxy.configured')


@when('fw-proxy.configured')
@when_not('fw-proxy.ready')
def fw_proxy_ready():
    status_set('active', 'ready')
    set_flag('fw-proxy.ready')


@when('actions.ping')
@when_not('fw-proxy.configured')
def pingme():
