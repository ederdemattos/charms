...
def test_connection():
    ...
    remove_flag('rt-proxy.ready')
    try:
        who, _ = run('whoami')
    except MgmtNotConfigured as e:
        remove_flag('rt-proxy.configured')
        status_set('blocked', str(e))
    except subprocess.CalledProcessError as e:
        remove_flag('rt-proxy.configured')
        status_set('blocked', e.output)
    else:
        set_flag('rt-proxy.configured')


@when('rt-proxy.configured')
@when_not('rt-proxy.ready')
def rt_proxy_ready():
    status_set('active', 'ready')
    set_flag('rt-proxy.ready')


@when('actions.ping')
@when_not('rt-proxy.configured')
def pingme():
